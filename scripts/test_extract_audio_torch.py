...
if __name__ == "__main__":
    ...
    wav_in = ap.load_wav(filepath)
    # extract spectrogram
    spectrogram, phase = ap.get_spec_from_audio(wav_in,  return_phase=True)
    
    spectrogram = torch.from_numpy(np.array([spectrogram, spectrogram]))
    phase = torch.from_numpy(np.array([phase, phase]))
    ...
    wav = ap.torch_inv_spectrogram(spectrogram, phase)
    wav = wav.cpu().detach().numpy()
    ap.save_wav(wav[0], '../test-spec-exctraction.wav')
    ap.save_wav(wav[1], '../test2-spec-exctraction.wav')
