...
for i in range(len(all_speakers)):
    ...
    for j in range(i+1,len(all_speakers[i:])):
            ...
            interference_wav = random.choice(wav_samples).replace(interference_speaker, '')
            while clean_wav == interference_wav: # its necessary for clean interference not is same text, its necessary because the texts in vctk is parallel
                interference_wav = random.choice(wav_samples).replace(speaker_clean, '')
